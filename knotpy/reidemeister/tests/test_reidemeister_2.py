...
from knotpy.reidemeister.reidemeister_2 import reidemeister_2_poke, reidemeister_2_unpoke, find_reidemeister_2_poke, find_reidemeister_2_unpoke
...
def test_r2_find_moves():
    ...
    for k, polynomial in diagrams.items():
        ...
        nr2 = sum((len(f) * (len(f)-1))//2 for f in list(k.faces))
...
def test_make_undo_reidemeister_1_move():
    ...
    for k, polynomial in diagrams.items():
        ...
        for loc in r2_locations:
            k_ = reidemeister_2_poke(k, loc, inplace=False)
            r1_un_locations = list(find_reidemeister_2_unpoke(k_))
            assert 1 <= len(r1_un_locations) <= 2
            k__ = reidemeister_2_unpoke(k_, r1_un_locations[0], inplace=False)
