from knotpy import all_orientations
...
def test_canonical_oriented():
    native_a = "a=V(b0 c0 d3) b=V(a0 d2 c1) c=X(a1 b2 d1 d0) d=X(c3 c2 b1 a2)"
    #ative_b = "m=V(y0 w0 x3) y=V(m0 x2 w1) w=X(m1 y2 x1 x0) x=X(w3 w2 y1 m2)"
    ...
    assert canonical(k_a) == canonical(k_b)
    assert canonical(k_b) != canonical(k_c)
...
def test_canonical_degenerate_oriented():
    ...
    b = all_orientations(from_knotpy_notation("a=V(a3 a2 a1 a0)"))

    ka = canonical(a)
    kb = [canonical(_) for _ in b]

    assert sanity_check(ka)
    assert all(sanity_check(_) for _ in b)
