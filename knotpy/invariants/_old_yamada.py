...
def yamada_polynomial(k: PlanarDiagram, normalize=True):
    ...
    k.framing = 0  # framing not supported
...
def _compute_yamada_unnormalized(k: PlanarDiagram, first_pass_use_cache=True):
    """
    Compute the unnormalized Yamada polynomial of a given planar diagram.
    """
...
def _yamada_from_cache(k: PlanarDiagram) -> Expr:
    ...
    # save states and framing and clear attributes
    A_states = k.attr.get("_A", 0)
    B_states = k.attr.get("_B", 0)
    framing =  k.attr.get("framing", 0)
    k.attr.clear()
    k = canonical(k)
    ...
    if k in _yamada_cache:
        ...
        _yamada_cache[k.copy()] = polynomial

    return polynomial * _A ** (A_states - B_states) * (-_A) ** int(- 2 * framing)  # adjust the framing and A/B coefficients of the input graph
...
if __name__ == "__main__":
    ...
    k = from_knotpy_notation("a → V(a1 a0)")
    print(yamada_polynomial(k, normalize=False))
    print(k)
    print(expand(_yamada_rec(k)))
    pass
