...
def to_notation_dispatcher(notation: str) -> str:
    assert isinstance(notation, str), f"The argument notation={notation} should be a string."
...
def from_notation_dispatcher(notation: str) -> str:
    assert isinstance(notation, str), f"The argument notation={notation} should be a string."
    notation = "".join(c for c in notation.lower() if c in string.ascii_lowercase)
    if notation not in _reversed_notation_aliases:
        raise ValueError(f"Unknown notation '{notation}'.")
