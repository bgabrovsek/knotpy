...
def _plot_arcs(k, circles, ax, bounding_box=False):
    ...
    :param ax: pyplot axis
    :param bounding_box: if true, also returns the bounding box as a 2nd return
    :return:
    """
...
def draw_from_layout(k,
    ...
    bb = BoundingBox()
    if bounding_box:
        new_vertices, bb = _plot_arcs(k, layout, ax, True)
    else:
        new_vertices = _plot_arcs(k, layout, ax, False)
