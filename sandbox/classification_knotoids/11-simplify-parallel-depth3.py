...
output = DATA_FOLDER / "knotoids-filter-5.txt"
header = None
...
if __name__ == "__main__":
    ...
    for chunk in kp.Bar(kp.load_collection_chunk_iterator(input, CHUNK_SIZE), total=ceil(num_diagrams/CHUNK_SIZE)):
        ...
        all_diagrams += sum(len(d) for d in bad_chunk)

        result = kp.simultaneously_simplify_diagrams_parallel(bad_chunk, 3, processes=cpu_count)

        new_diagrams += sum(len(d) for d in result)

        kp.Bar.set_comment(f"{all_diagrams} -> {new_diagrams}")
