...
input = DATA_FOLDER / "knotoids-filter-2-no-mirrors.txt"
output = DATA_FOLDER / "knotoids-filter-3.txt"
...
def remove_obvious_sums(knots):
    ...
    for k in knots:
        is_sum = len(kp.bridges(k)) > 2 or kp.is_connected_sum(k)  # has three bridges (two leafs and the other one or is sum)
...
comment = "Knotoids up to 8 crossings without mirrors simplified once in parallel without sums and mirrors again // 9.8.2024"
